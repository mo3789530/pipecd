...
import (
	...
	"github.com/pipe-cd/pipecd/pkg/app/pipedv1/livestatestore/opentofu"
...
func NewDetector(
	...
	logger = logger.Named("opentofu-detector").With(
...
func (d *detector) Run(ctx context.Context) error {
	d.logger.Info("start running drift detector for opentofu applications")
	...
	for {
		...
		case <-ctx.Done():
			d.logger.Info("drift detector for opentofu applications has been stopped")
			return nil
		}
	}
}
...
func (d *detector) checkApplication(ctx context.Context, app *model.Application, repo git.Repo, headCommit git.Commit) error {
	...
	// Load config
	cpCfg := d.provider.OpenTofuConfig
